...
def add_address_to_list(user_id, site_name, address):
    filename = f'address_lists/{site_name}/{user_id}_address_list.txt'
...
def remove_address_from_list(user_id,site_name, address):
    filename = f'address_lists/{site_name}/{user_id}_address_list.txt'
    with open (filename, 'a') as file:
        file.close()     
    with open (filename, 'r') as file:
...
def coin_checker(user_id):
    messages = []
    for site in url:
        try:
            filename = f'address_lists/{site}/{user_id}_address_list.txt'  
            with open (filename, 'r') as file:
                address_list = file.readlines()
                address_list = [address.strip() for address in address_list]
            if site != 'bscscan':
...
def transaction_checker(user_id):
    messages = []
    for site in url:
        try:
            filename = f'address_lists/{site}/{user_id}_address_list.txt' 
